...
[xntgEXPT,tt,nucleusNames,geneNames] = ...
                loadNCBIFiles ('dream-fivegenenet1-xntg.txt', ...
                               'dream-fivegenenet1-tt.txt');

%======== READ VALUES OF OPTIONS p, v, and x THAT HAVE BEEN TUNED BY USER  =======
% INDEX ORDER IS n, g, o (nucleus, gene, option index)
pvxOpts_ngo = loadMDA ('dream-fivegenenet1-options.mda');
...
opts = struct(  'debug', 0, ...
    ...
    'lambda', 0.1, ...
    'lm', 'lassoglm'); % glmfit|FIGRlogReg|lassoglm

%======== set the ODE options
ODEopts = odeset('AbsTol', opts.ODEAbsTol);

numGenes = 5;  % have to hardwire
...
saveMDA ("dream-fivegenenet1-yntg.mda", yntgEXPT);

%======== REFINE GRN BY NELDER-MEAD ========
xntgFLAT = reshape(xntgEXPT, 11, 5);
...
% save the T matrix
filename = 'dream-fivegenenet1-Tmatrix.txt';
saveTMatrix (filename, grnREF.Tgg, geneNames);

% save the plots
exportgraphics(gcf,'dream-fivegenenet1-fits.pdf','BackgroundColor','none');
