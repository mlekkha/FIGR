function [init_chisq, init_rms, fh_chisq, init_paramvec] = ...
    ...
    % declare a local GRN struct so that we're not creating it over and
    % over again
    lgrn = grn;

    % set up the computeTrajs function so we're not doing this over and
    % over again in getChiSquare
    computeTrajs_fh = str2func(opts.computetrajsfunction);

    % pack the parameter structure into an initial parameter vector
    init_paramvec = packParams(lgrn, numGenes);
    ...
    function chisq = getChiSquare(paramvec)
        ...
        % Call computeTrajs_fh to solve the equations with given values of
        % parameters
        [xntgMODEL] = computeTrajs_fh (opts, lgrn, lxntgEXPT, ltt);


        % Compute chi-square. Exclude the first timepoint, since it is the
        % initial condition
        diff = xntgEXPT(:,2:end,1:numGenes) - xntgMODEL(:,2:end,1:numGenes);
        chisq = sum(diff.*diff, 'all');
